...
#'
#' In PDF mode (if pdf_title or output_file is provided) the plots are printed to a PDF.
#' In interactive mode (if both pdf_title and output_file are NULL) the plots are captured using recordPlot()
...
#' @param output_file Optional. A file name for output (PDF mode); if provided, this overrides pdf_title.
...
cyt_pca <- function(data, group_col = NULL, group_col2 = NULL,
  ...
  if (group_col == group_col2) {
    ...
    pca_result <- mixOmics::pca(the_data_df, ncomp = comp_num,
    ...
    # Individuals plot.
    if (pdf_mode) {
      mixOmics::plotIndiv(pca_result,
                          group = the_groups, ind.names = FALSE, legend = TRUE,
                          col = colors, title = paste("PCA:", overall_analysis),
    ...
    scree_data <- data.frame(Component = 1:comp_num,
                             Variance = variances,
    ...
    scree_plot <- ggplot2::ggplot(scree_data, ggplot2::aes(x = Component)) +
      ...
      ggplot2::geom_line(ggplot2::aes(y = Cumulative, color = "Cumulative"),
                         size = 1, linetype = "dashed") +
      ggplot2::geom_point(ggplot2::aes(y = Cumulative, color = "Cumulative"), size = 2) +
      ggplot2::scale_color_manual(values = c("Individual" = "blue", "Cumulative" = "green")) +
      ggplot2::labs(title = paste("Scree Plot:", overall_analysis),
                    x = "Principal Components", y = "Explained Variance", color = "Variance Type") +
      ggplot2::theme_minimal() +
      ggplot2::scale_x_continuous(breaks = 1:comp_num) +
      ggplot2::geom_text(ggplot2::aes(y = Variance, label = paste0(round(Variance * 100, 1),"%")),
                         vjust = -1.5, hjust = 0.5, size = 4) +
      ggplot2::geom_text(ggplot2::aes(y = Cumulative, label = paste0(round(Cumulative * 100, 1),"%")),
                         vjust = 1.5, hjust = 0.5, size = 4)
    ...
    if (pdf_mode) {
      ...
      print(scree_plot)
      result_list$overall_scree_plot <- recordPlot()
    }
    if (!is.null(progress)) {
    ...
    for (comp in 1:comp_num) {
      if (pdf_mode) {
        mixOmics::plotLoadings(pca_result,
                               comp = comp, size.names = 1, size.legend = 1,
                               legend.color = colors,
    ...
    if (pdf_mode) {
      mixOmics::plotVar(pca_result,
                        comp = c(1, 2), var.names = TRUE, cex = 4, col = "black",
                        overlap = TRUE, title = paste("Correlation Circle Plot:", overall_analysis),
                        style = "ggplot2")
    } else {
    ...
    for (lev in levels_vec) {
      ...
      scree_plot <- ggplot2::ggplot(scree_data, ggplot2::aes(x = Component)) +
        ...
        ggplot2::geom_point(ggplot2::aes(y = Cumulative, color = "Cumulative"), size = 2) +
        ggplot2::scale_color_manual(values = c("Individual" = "blue", "Cumulative" = "green")) +
        ggplot2::labs(title = paste("Scree Plot:", title_sub),
                      x = "Principal Components", y = "Explained Variance", color = "Variance Type") +
        ggplot2::theme_minimal() +
        ggplot2::scale_x_continuous(breaks = 1:comp_num) +
        ggplot2::geom_text(ggplot2::aes(y = Variance, label = paste0(round(Variance * 100, 1),"%")),
      ...
      if (pdf_mode) {
        ...
        dev.control(displaylist = "enable")
        print(scree_plot)
        sublist$overall_scree_plot <- recordPlot()
      }
      ...
      if (pdf_mode) {
        mixOmics::plotVar(pca_result,
                          comp = c(1, 2), var.names = TRUE, cex = 4, col = "black",
                          overlap = TRUE, title = paste("Correlation Circle Plot:", title_sub),
                          style = "ggplot2")
      } else {
