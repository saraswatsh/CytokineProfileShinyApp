mod_save_key_inputs_server <- function(input, output, session, app_ctx) {
  ...
  shiny::observe({
    ...
    cols <- input_values$selected_categorical_cols %||% character(0)
    filter_values <- userState$step2_filter_values %||% list()
    filter_values <- filter_values[intersect(names(filter_values), cols)]

    for (col in cols) {
      filter_id <- paste0("filter_", col)
      value <- input_values[[filter_id]]
      if (!is.null(value)) {
        filter_values[[col]] <- value
      }
    }

    if (!identical(filter_values, userState$step2_filter_values)) {
      userState$step2_filter_values <- filter_values
    }
  })
  ...
  lapply(names(analysis_font_specs), function(func_name) {
    ...
    shiny::observeEvent(input[[paste0(spec$prefix, "_font_apply")]], {
      ...
      shiny::showNotification("Text sizes applied.", type = "message")
    }, ignoreInit = TRUE)
  })
  shiny::observeEvent(input$next4, {
    ...
    userState[[spec$state_key]] <- font_settings_state_from_inputs(
      input = input,
      prefix = spec$prefix,
      supported_fields = spec$supported_fields,
      default_font_settings = spec$default_font_settings
