...
cyt_heatmap <- function(
  ...
  if (tolower(ext) == "png") {
    ...
    grDevices::pdf(file = output_file, width = 7, height = 7)
  } else {
  ...
  heatmap_args <- list(
    ...
    hclustfun = function(x) stats::hclust(x, method = "complete"),
    dendrogram = "both",
    trace = "column",
    key = TRUE,
    cexCol = 1,
  ...
  if (!is.null(annotation_col_name) && annotation_col_name %in% names(data)) {
    ...
    if (length(ann_data) >= ncol(numeric_data)) {
      ...
      side_colors <- grDevices::rainbow(
        num_levels
      )[as.integer(ann_data[seq_len(ncol(
        numeric_data
      ))])]
      heatmap_args$ColSideColors <- side_colors
    } else {
      warning(
        "Length of annotation column is less than number of numeric columns. Skipping annotation colors."
      )
    }
  }
  ...
  if (!is.null(progress))
    progress$inc(0.05, detail = "Returning output file path")
  return(output_file)
}
