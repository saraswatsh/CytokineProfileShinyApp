...
vignette: >
  %\VignetteIndexEntry{Understanding Heatmaps}
...
2. Check whether clustering puts samples from the same group near one another.
3. Identify cytokine clusters that appear to rise or fall together.
4. Use annotations to see whether those patterns align with biology, treatment, or batch structure.
...
4. Set `Annotation Column` and `Annotation Side` so the clustering can be interpreted in study context.
5. Inspect the figure both with and without clustering if you want to separate raw ordering from similarity structure.

## What to read next

Related articles:

* [Understanding Correlation Analysis](Understanding-Correlation-Analysis.html) for targeted association screening.
* [Understanding PCA](Understanding-PCA.html) for a low-dimensional view of structure.
* [Understanding Boxplots and Violin Plots](Understanding-Boxplots-and-Violin-Plots.html) when a cluster needs raw-distribution follow-up.
