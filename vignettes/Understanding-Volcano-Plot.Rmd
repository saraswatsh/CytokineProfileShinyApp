...
* X-axis: Average log2 fold change (`log2FC`)
  Positive values indicate higher abundance in the first condition of the comparison, while negative values indicate lower abundance.
* Y-axis: `-log10(p-value)`
  Larger values indicate smaller p-values. Points higher on the plot therefore have stronger statistical evidence against no difference.

### The visual cues

* Each point is one cytokine.
* The vertical dashed lines mark the fold-change threshold. In this example, `log2FC = -1` and `log2FC = 1` correspond to a 2-fold decrease or increase.
* The horizontal dashed line marks the p-value threshold. In this example, `-log10(p-value) = 2`, which corresponds to `p = 0.01`.
...
* `Condition 1` and `Condition 2`: decide which two levels of that column are contrasted.
...
4. Adjust `Log2 Fold Change Threshold`, `P-Value Threshold`, and `Top Labels` only if needed.
5. Review the highlighted points together with the labels and any follow-up plots.
...
* [Understanding Dual-Flashlight Plot](Understanding-Dual-Flashlight-Plot.html) for threshold-based effect-size screening without p-value emphasis.
