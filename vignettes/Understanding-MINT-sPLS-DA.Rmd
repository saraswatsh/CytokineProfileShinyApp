---
title: "Understanding MINT sPLS-DA"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Understanding MINT sPLS-DA}
...
If you do not actually have multiple study sources, standard `Sparse Partial Least Squares - Discriminant Analysis (sPLS-DA)` is usually simpler and easier to interpret.
...
If a `MINT sPLS-DA Stratification Column` is selected, the app can also repeat the analysis within each subgroup and show nested result tabs.
...
* `Draw Ellipse`, `Plot ROC Curve`, and `Draw Background`: optional aids for interpretation and performance checking.

For most users, `MINT sPLS-DA Comparison Column`, `Batch Column`, and `Number of Variables to Select` are the settings that matter most.
...
When enabled, the clustered image map helps you see whether the selected variables show recognizable group patterning.

This is useful for communication, but it should support the model interpretation rather than replace it.

### ROC Curve

If the ROC view is available, use it as a check on discrimination quality rather than trusting the score plots alone.
...
* apparent separation can still reflect residual batch structure if the study sources are very different
...
1. Filter the dataset to the biological groups and samples you want to compare.
2. Choose `Multivariate INTegration Sparse Partial Least Squares - Discriminant Analysis (MINT sPLS-DA)`.
...
6. Optionally add `MINT sPLS-DA Stratification Column`, `Draw a Clustered Image Map?`, `Plot ROC Curve`, or `Draw Ellipse`.
7. Read the global and partial plots together before trusting the selected variables.

## What to read next

Related articles:

* [Understanding (s)PLS-DA](Understanding-sPLS-DA.html) for supervised class separation without explicit multi-study integration.
* [Understanding PLSR](Understanding-PLSR.html) if your response is numeric instead of categorical.
* [Understanding PCA](Understanding-PCA.html) for a simpler unsupervised first look at structure.
